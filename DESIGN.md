...
### SEO 雙標題機制
- `articles-data.ts` 的 `Article` 型別有 `seoTitle?` 欄位
- `seoTitle` 用於 `<title>` 標籤和 OpenGraph（Google 搜尋結果顯示的）
- `title` 用於頁面 H1（讀者看到的）
- `generateMetadata` 優先用 `seoTitle`，沒有就 fallback 到 `title`
- 目前 10 篇文章有 seoTitle（健檢紅字系列 + 高搜尋量主題）

### AI 可讀性
- `public/llms.txt`：給 AI 爬蟲的網站結構摘要（主要頁面、五種代謝類型、工具、概念）
- `robots.txt`：全開放，沒有封鎖任何 AI 爬蟲
...
*最後更新：2026-04-04，commit 6bac573*
