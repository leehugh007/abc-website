    ...
    <>
      ...
      <section className="pt-14 pb-10 px-5">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-3 text-sm text-[#a8a29e] mb-8">
      ...
      <section className="pb-10 px-5">
        <div className="max-w-2xl mx-auto">
          <div className="rounded-xl border border-[#eee9e5] bg-white p-6">
            ...
            <nav className="space-y-2 text-sm">
              {[
                ...
                <a
                  key={item.id}
                  href={`#${item.id}`}
                  className="block text-[#6b6560] hover:text-[#2a9d6f] transition-colors"
                >
                  {item.label}
                </a>
              ))}
      ...
      <article className="pb-16 px-5">
        <div className="max-w-2xl mx-auto prose prose-lg prose-stone prose-headings:font-bold prose-headings:tracking-tight prose-h2:text-xl prose-h2:mt-12 prose-h2:mb-4 prose-h3:text-lg prose-h3:mt-8 prose-h3:mb-3 prose-p:leading-[1.9] prose-p:text-[#2a2520] prose-li:text-[#2a2520] prose-blockquote:border-l-[#2a9d6f] prose-blockquote:bg-[#f3f9f5] prose-blockquote:rounded-r-lg prose-blockquote:not-italic prose-strong:text-[#1a5e3f]">
          ...
          <p>這個現象叫做<strong>代謝適應</strong>。它不是你的敵人，它是你的身體在保護你。問題是，「少吃多動」這個方法，<Link href="/articles/eat-less-move-more-myth" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]">從一開始就跟你的身體作對</Link>。</p>
          ...
          <p>這就是<Link href="/articles/dark-neck-insulin" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]"><strong>胰島素阻抗</strong></Link>。</p>
          ...
          <p>所以你吃很少還是瘦不下來，下午一定要來杯手搖飲，<Link href="/articles/normal-blood-sugar-trap" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]">體檢數字看起來還行但腰圍一直增加</Link>——這些全都跟胰島素有關。</p>
          ...
          <div className="not-prose my-8">
            <div className="rounded-2xl bg-[#1a5e3f] text-white p-8">
          ...
          <p>想更完整了解 ABC 方法的細節，可以看<Link href="/method" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]">方法介紹頁</Link>。</p>
          ...
          <div className="not-prose my-8 space-y-3">
            {[
              { emoji: "🔴", name: "高轉速型", desc: "壓力太大、皮質醇偏高，脂肪往肚子堆。你不是不健康，是太拼了。", slug: "high-rpm", color: "#e74c3c" },
              { emoji: "🟠", name: "燃燒殆盡型", desc: "把能量都給了別人，自己的代謝引擎快熄火。你不是不在乎自己。", slug: "burnout", color: "#e67e22" },
              { emoji: "🟡", name: "雲霄飛車型", desc: "血糖一天坐三次大怒神，精神和體重都跟著上上下下。", slug: "roller-coaster", color: "#f39c12" },
              { emoji: "🟢", name: "省電模式型", desc: "長期節食把代謝拖慢，吃很少還是瘦不下來。你不是吃太多，是吃太少太久了。", slug: "power-save", color: "#7f8c8d" },
              { emoji: "🔵", name: "穩定燃燒型", desc: "代謝引擎運轉得不錯，維持好習慣就好。", slug: "steady", color: "#27ae60" },
            ].map((type) => (
              <Link
                key={type.slug}
                href={`/types/${type.slug}`}
                className="flex items-start gap-4 p-5 rounded-xl bg-white border border-[#eee9e5] border-l-4 hover:shadow-sm transition-all group no-underline"
                style={{ borderLeftColor: type.color }}
              >
                <span className="text-2xl shrink-0">{type.emoji}</span>
                <div>
                  <p className="font-bold text-base text-[#2a2520] group-hover:text-[#2a9d6f] transition-colors">{type.name}</p>
                  <p className="text-sm text-[#6b6560] mt-0.5">{type.desc}</p>
                </div>
              </Link>
            ))}
          </div>

          <p>不確定自己是哪種？<Link href="/quiz" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]">做個 30 秒測驗</Link>，馬上知道。</p>
          ...
          <p>三年來超過 3000 位學員走過這個流程。<Link href="/articles/socks-story-62kg" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]">有人瘦了 62 公斤</Link>，<Link href="/articles/chicken-soup-love-story" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]">有人血壓恢復正常</Link>，有人終於穿回婚前的裙子。每個人的起點不同，但邏輯是一樣的：搞懂代謝，瘦是順便的事。</p>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-[#f8faf7]">
                  <th className="text-left p-3 font-bold border-b border-[#eee9e5]"></th>
                  <th className="text-left p-3 font-bold border-b border-[#eee9e5]">節食 / 少吃多動</th>
                  <th className="text-left p-3 font-bold border-b border-[#eee9e5]"><Link href="/articles/ozempic-alternative-abc" className="text-[#8e44ad] no-underline hover:underline">瘦瘦針</Link></th>
                  <th className="text-left p-3 font-bold border-b border-[#eee9e5] text-[#2a9d6f]">ABC 代謝重建</th>
                </tr>
              </thead>
              <tbody className="text-[#6b6560]">
                <tr><td className="p-3 border-b border-[#eee9e5] font-medium text-[#2a2520]">原理</td><td className="p-3 border-b border-[#eee9e5]">減少熱量攝取</td><td className="p-3 border-b border-[#eee9e5]">藥物壓抑食慾</td><td className="p-3 border-b border-[#eee9e5] text-[#2a2520] font-medium">修復代謝系統</td></tr>
                <tr><td className="p-3 border-b border-[#eee9e5] font-medium text-[#2a2520]">短期效果</td><td className="p-3 border-b border-[#eee9e5]">快（前 2 週）</td><td className="p-3 border-b border-[#eee9e5]">很快</td><td className="p-3 border-b border-[#eee9e5] text-[#2a2520]">中等（4-6 週見效）</td></tr>
                <tr><td className="p-3 border-b border-[#eee9e5] font-medium text-[#2a2520]">復胖率</td><td className="p-3 border-b border-[#eee9e5]">極高（95%）</td><td className="p-3 border-b border-[#eee9e5]">高（停藥後 2/3 回來）</td><td className="p-3 border-b border-[#eee9e5] text-[#2a2520] font-medium">低（學會的就是你的）</td></tr>
                <tr><td className="p-3 border-b border-[#eee9e5] font-medium text-[#2a2520]">需要忍耐嗎</td><td className="p-3 border-b border-[#eee9e5]">很需要</td><td className="p-3 border-b border-[#eee9e5]">不用（藥物代替）</td><td className="p-3 border-b border-[#eee9e5] text-[#2a2520] font-medium">前 2 週需要，之後不用</td></tr>
                <tr><td className="p-3 font-medium text-[#2a2520]">代謝影響</td><td className="p-3">代謝變慢</td><td className="p-3">不改善代謝</td><td className="p-3 text-[#2a2520] font-medium">修復代謝</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <h3>外食族可以做嗎？</h3>
          <p>完全可以。核心就是<Link href="/articles/eating-order-blood-sugar" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]">「菜肉飯順序法」</Link>——不管吃什麼，先吃菜、再吃肉、最後吃飯。火鍋是外食首選，便當也可以。</p>
          ...
          <p>更多問題可以看<Link href="/faq" className="text-[#2a9d6f] underline decoration-[#2a9d6f]/35 hover:decoration-[#2a9d6f]">完整 FAQ（15 題）</Link>。</p>
        ...
        <div className="max-w-xl mx-auto">
          <div className="grid sm:grid-cols-3 gap-3">
            ...
            >
              ...
              <span className="font-bold group-hover:text-[#2a9d6f] transition-colors">測出代謝類型</span>
            ...
            <a
              ...
              className="flex flex-col items-center gap-2 p-6 rounded-xl border border-[#eee9e5] hover:border-[#06C755] hover:shadow-sm transition-all text-center group"
            >
              <span className="text-3xl">💬</span>
              <span className="font-bold group-hover:text-[#06C755] transition-colors">問一休</span>
              <span className="text-xs text-[#a8a29e]">加入 LINE 直接聊</span>
            </a>
            <Link
              href="/articles"
              className="flex flex-col items-center gap-2 p-6 rounded-xl border border-[#eee9e5] hover:border-[#2a9d6f] hover:shadow-sm transition-all text-center group"
            >
              <span className="text-3xl">📖</span>
              <span className="font-bold group-hover:text-[#2a9d6f] transition-colors">看更多文章</span>
              <span className="text-xs text-[#a8a29e]">18 篇免費閱讀</span>
            </Link>
          </div>
