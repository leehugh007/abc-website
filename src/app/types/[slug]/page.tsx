    ...
    <>
      ...
      <section className="py-12 px-5">
        <div className="max-w-2xl mx-auto">
          <p
            className="text-lg leading-relaxed text-ink border-l-4 pl-5 font-semibold mb-8"
      ...
      <section className="pb-12 px-5">
        <div className="max-w-2xl mx-auto space-y-12">
          {type.sections.map((section, i) => (
            <div key={i}>
              <div className="flex items-center gap-3 mb-4 pb-3 border-b-2 border-edge">
                <span className="text-2xl">{section.icon}</span>
                <h2 className="text-xl font-bold">{section.title}</h2>
              </div>
              <div className="text-base leading-[1.8] text-ink whitespace-pre-line">
                {section.text}
              </div>
            </div>
          ))}
      ...
      {type.highlight && (
        <section className="py-8 px-5">
          <div className="max-w-2xl mx-auto">
            <div className="bg-[#fef8ee] border-l-4 border-amber rounded-r-xl p-6 text-[17px] font-semibold leading-relaxed">
              {type.highlight}
            </div>
          </div>
        </section>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-3">
            {type.suggestions.map((sug, i) => (
              <div
                key={i}
                className="p-4 rounded-xl bg-white border border-edge text-[15px] leading-relaxed"
              >
                <span
      ...
      <section className="py-16 px-5 bg-white">
        <div className="max-w-xl mx-auto text-center">
          <p className="text-subtle mb-2">你不用一個人搞懂這些</p>
          <h2 className="text-xl font-bold mb-6">
          ...
          <p className="text-xs text-muted mt-4">3,000+ 人已經透過 ABC 重建代謝</p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {TYPES.filter((t) => t.slug !== type.slug).map((t) => (
              <Link
                key={t.slug}
                href={`/types/${t.slug}`}
                className="text-center p-4 rounded-xl border border-edge hover:border-edge-dark hover:shadow-sm transition-all"
              >
